...
function testDecodeTransaction() {
    ...
    console.assert(decoded.version === '02000000', `Expected version to be 2, but got ${decoded.version}`);
    console.assert(decoded.marker === 0, `Expected marker to be 0, but got ${decoded.marker}`);
    console.assert(decoded.flag === 1, `Expected flag to be 0, but got ${decoded.flag}`);
    console.assert(decoded.input_count === 1, `Expected num_inputs to be 1, but got ${decoded.input_count}`);
    console.assert(decoded.inputs_arr.length === decoded.input_count, `Expected ${decoded.input_count} inputs, but got ${decoded.inputs_arr.length}`);
    console.assert(decoded.output_count === 2, `Expected num_outputs to be 2, but got ${decoded.output_count}`);
    console.assert(decoded.Output_arr.length === decoded.output_count, `Expected ${decoded.output_count} outputs, but got ${decoded.Output_arr.length}`);
    console.assert(decoded.witness_count === 1, `Expected witness_count to be 1, but got ${decoded.witness_count}`);
    console.assert(decoded.witness_arr.length === decoded.witness_count, `Expected ${decoded.witness_count} witnesses, but got ${decoded.witness_arr.length}`);
    console.assert(decoded.lockTime === 0, `Expected locktime to be 00000000, but got ${decoded.lockTime}`);
